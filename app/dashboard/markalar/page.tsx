...
export default function MarkalarsPage() {
  ...
  const markalar = useMemo(() => {
    ...
    return ranked.map((m, bazRank) => ({
      ...
      bazRank:    bazRank + 1,
      cmpRank:    cmpRanked.findIndex(x => x.marka === m.marka) + 1, // 0 = yok
    })).sort((a, b) => {
          ...
          <div>
            ...
            <div style={{ background: 'var(--surf)', border: '1px solid var(--bd)', borderRadius: 10, overflow: 'hidden', marginBottom: 16 }}>
              ...
              >
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 10 }}>
                  ...
                  <tbody>
                    {markalar.map((m, idx) => {
                      // Sıra değişimi: cmpRank - bazRank (pozitif = yükselme)
                      const rankDelta = (selCmpDonem && m.cmpRank > 0)
                        ? m.cmpRank - (idx + 1)
                        : null
                      const rankColor = rankDelta === null ? 'var(--tx3)'
                        : rankDelta > 0 ? '#10b981'
                        : rankDelta < 0 ? '#ef4444'
                        : '#64748b'
                      return (
            ...
            <div style={{ background: 'var(--surf)', border: '1px solid var(--bd)', borderRadius: 10, padding: '16px 20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                ...
                <button onClick={() => setSortKpi(-1)} style={{ padding: '5px 14px', borderRadius: 20, fontSize: 10, fontWeight: 700, cursor: 'pointer', border: `1px solid ${sortKpi === -1 ? '#f59e0b' : 'var(--bd)'}`, background: sortKpi === -1 ? 'rgba(245,158,11,.12)' : 'var(--surf2)', color: sortKpi === -1 ? '#f59e0b' : 'var(--tx3)' }}>★ Genel Skor</button>
              </div>
