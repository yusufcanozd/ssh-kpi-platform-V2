          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11, tableLayout: 'auto' }}>
            <thead>
              <tr style={{ background: 'var(--surf2)', position: 'sticky', top: 0, zIndex: 3 }}>
                ...
                <th style={{ ...thS, position: 'sticky', right: selCmpDonem ? 50 : 0, background: 'var(--surf2)', minWidth: 60 }}>
                  Skor
                  {selCmpDonem && <div style={{ fontSize: 7, fontWeight: 400, color: 'var(--tx3)' }}>{selCmpDonem}</div>}
                </th>
                {selCmpDonem && <th style={{ ...thS, position: 'sticky', right: 0, background: 'var(--surf2)', minWidth: 50, textAlign: 'center' }}>Δ Sıra</th>}
              </tr>
            </thead>
            <tbody>
              {markalar.map((m, i) => {
                ...
                return (
                  <tr key={m.marka} style={{ borderBottom: '1px solid var(--bd)' }}>
                    ...
                    <td style={{ ...tdS, position: 'sticky', right: selCmpDonem ? 50 : 0, background: 'var(--surf)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        ...
                        {selCmpDonem && scoreDiff !== null && (
                          <span style={{ fontSize: 9, fontWeight: 600, color: scoreDiff > 0 ? '#10b981' : scoreDiff < 0 ? '#f87171' : 'var(--tx3)' }}>
