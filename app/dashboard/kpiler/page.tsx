...
export default function KpilerPage() {
  ...
  const riskLabel: Record<string,string> = { no_ranking:'Kısıtlı' }
    ...
    <div className={styles.wrap}>
      ...
      <div className={styles.content}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:14}}>
          {KPI_CONFIG.map((k, i) => (
            <div key={k.key} onClick={() => setSelKpi(i)}
              style={{background:'var(--surf)',border:`1px solid ${selKpi===i?'var(--blue)':'var(--bd)'}`,borderRadius:9,padding:12,cursor:'pointer',transition:'all .12s',background:selKpi===i?'rgba(59,130,246,.06)':'var(--surf)'}}>
              ...
              {k.compliance && <span style={{display:'inline-block',fontSize:8,fontWeight:700,padding:'2px 6px',borderRadius:4,marginTop:4,background:'rgba(239,68,68,.12)',color:'#f87171'}}>{riskLabel[k.compliance]||k.compliance}</span>}
            </div>
          ))}
        </div>
