    ...
    return (
      <Bar data={{
        ...
        datasets: [{
          label: selDonem || 'Baz',
          data: bazData,
          backgroundColor: labels.map(function(_, i) { return i === 0 ? 'rgba(251,191,36,.15)' : 'rgba(59,130,246,.15)' }),
          borderColor:     labels.map(function(_, i) { return i === 0 ? '#fbbf24' : '#3b82f6' }),
          borderWidth: 2, borderRadius: 5,
        }].concat(selCmpDonem ? [{
          label: selCmpDonem,
          data: cmpData,
          backgroundColor: labels.map(function(_, i) { return i === 0 ? 'rgba(251,191,36,.5)' : 'rgba(59,130,246,.5)' }),
          borderColor:     labels.map(function(_, i) { return i === 0 ? '#fbbf24' : '#3b82f6' }),
  ...
  return (
    <div className={styles.wrap}>
      ...
      <div className={styles.content}>
        ...
        {/* Üst bölge skor kartları */}
        <div style={{display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:8, marginBottom:14}}>
          {bolgeList.map(function(b) {
            const bazG = bolgeData.find(function(d) { return d.bolge === b })?.bazScore?.genel || 0
            const cmpG = bolgeData.find(function(d) { return d.bolge === b })?.cmpScore?.genel || null
            const chg  = chgPct(bazG, cmpG)
            const isAct = selBolge === b
            return (
              <div key={b} style={{background: 'var(--surf2)', borderRadius:8, padding:'10px 8px', textAlign:'center',
                border: '1px solid ' + scoreColor(bazG) + '55'}}>
                <div style={{fontSize:8, fontWeight:700, color:scoreColor(bazG), marginBottom:4, lineHeight:1.3}}>{b}</div>
                <div style={{fontSize:20, fontWeight:800, fontFamily:'var(--font-dm-mono)', color:scoreColor(bazG), lineHeight:1}}>{bazG || '-'}</div>
                {cmpG !== null && <div style={{fontSize:10, color:'var(--tx3)', marginTop:1}}>{cmpG}</div>}
                {chg !== null && (
                ...
                <div style={{background:'rgba(0,0,0,.1)', borderRadius:3, height:3, overflow:'hidden', marginTop:6}}>
        ...
        {activeTab === 'markaSkor' && (
          <div>
                ...
                <table style={{width:'100%', borderCollapse:'collapse', fontSize:11}}>
                  <thead>
                    <tr style={{background:'var(--surf2)', position:'sticky', top:0, zIndex:3}}>
                      ...
                      <th style={{...thS, textAlign:'center', minWidth:70}}>Genel Skor</th>
                    </tr>
                  </thead>
                  <tbody>
                    {markalar.map(function(m, i) {
                      const trVal = getKpiScores(m.segment, '', selYas, selDonem)[selKpi] || 0
                      return (
                        <tr key={m.marka} style={{borderBottom:'1px solid var(--bd)'}}>
                          ...
                          <td style={{...tdS, textAlign:'center', fontFamily:'var(--font-dm-mono)', fontWeight:700, color:'#fbbf24'}}>{trVal}</td>
                          {bolgeList.map(function(b) {
                            const bv = getKpiScores(m.segment, b, selYas, selDonem)[selKpi] || 0
                            const hc = heatColor(bv, trVal, true)
                            return (
                              <td key={b} style={{...tdS, textAlign:'center', background:hc.bg}}>
                                <div style={{fontFamily:'var(--font-dm-mono)', fontSize:11, fontWeight:700, color:hc.color}}>{bv}</div>
                              </td>
                            )
                          })}
                          <td style={{...tdS, textAlign:'center'}}>
                            <span style={{fontFamily:'var(--font-dm-mono)', fontSize:11, fontWeight:700,
                              color:m.score>=80?'#10b981':m.score>=65?'#3b82f6':m.score>=50?'#f59e0b':'#ef4444'}}>{m.score}</span>
                          </td>
                        </tr>
                      )
                    })}
            ...
            <div className={styles.card}>
              <div className={styles.cardHd}><h3>{meta.ad} — Marka KPI Puan Karsilastirmasi</h3></div>
              <div style={{overflowX:'auto'}}><div style={{minWidth:markalar.slice(0,20).length*52, height:240}}>
                {barChart(markalar.slice(0,20).map(function(m) { return m.marka }),
                  barMarkaScoreBaz, barMarkaScoreCmp,
                  Math.max.apply(null, barMarkaScoreBaz.concat(barMarkaScoreCmp).concat([10])), 'int', true)}
              </div></div>
            </div>
          </div>
        )}
