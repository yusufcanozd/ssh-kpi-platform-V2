...
export default function BolgelerPage() {
  ...
  const isAllYas = !selYas || selYas.length < 4
  const yasLabel = isAllYas ? 'Tum Yas' : selYas + 'y'
  const flParts = [selSeg || 'Tum Seg.', yasLabel, selDonem || 'Tum Donem']
  const filterLabel = flParts[0] + ' - ' + flParts[1] + ' - ' + flParts[2]
              ...
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}>
                <thead>
                  <tr style={{background:'var(--surf2)'}}>
                    <th style={thS}>Bölge</th>
                    {KPI_META.map((k,ki)=>(
                      <th key={k.no} onClick={()=>setSelKpi(ki)}
                        style={{...thS,textAlign:'center',cursor:'pointer',minWidth:68,whiteSpace:'normal',wordBreak:'break-word',
                          color:selKpi===ki?'var(--blue)':'var(--tx3)',
                          background:selKpi===ki?'rgba(59,130,246,.06)':'var(--surf2)'}}>
