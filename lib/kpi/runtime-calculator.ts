...
export function createRuntimeCalculator(runtime: KpiRuntimeData | CubeRow[]): RuntimeCalculator {
  ...
  const getScoreWithMode = (
    ...
    return calculateScoreFromKpis(kpis, referenceKpis(seg, bolge, yas, donem, referenceMode), zeroVarianceIndexes)
  }
  ...
  return {
    ...
    getSegmentScores: (segments, bolge = '', yas = 'Tümü', donem = '', cmpDonem = '') => segments.map(seg => ({
      seg,
...
function calculateScoreFromKpis(
  ...
  for (const category of KAT_YAPILAR) {
    ...
    for (const kpiIdx of category.kpis) {
      const normalized = normalizeRaw(kpis[kpiIdx], refKpis[kpiIdx], kpiIdx, zeroVarianceIndexes)
...
function normalizeRaw(
  ...
  if (reference === 0) {
    return { score: lowerIsBetter ? (value === 0 ? 100 : 0) : 0, coverageIncluded: true }
  }
